...
const tags = [
  "AI & Data Science", "Privacy-First", "Creative Automation", 
  "Motion Graphics", "UI Design", "Prompt Engineering", 
  "Open Source", "Hackathon Winner"
];
...
export default function About() {
  return (
    <section id="about" className="py-32 px-6 max-w-4xl mx-auto">
      ...
      >
        <motion.span variants={itemVariants} className="font-mono text-accent text-sm tracking-widest uppercase">
          {`// about`}
        </motion.span>
        ...
        <motion.div variants={containerVariants} className="flex flex-wrap gap-3 mt-4">
          {tags.map((tag) => (
            <motion.span 
              key={tag}
              variants={itemVariants}
              className="px-4 py-2 bg-surface text-text-muted font-mono text-sm border border-accent/20 rounded-sm hover:-translate-y-1 hover:text-accent hover:border-accent hover:glow-amber transition-all duration-300"
            >
              {tag}
            </motion.span>
          ))}
